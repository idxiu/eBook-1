  ...
  <ebook-dialog :title="title" ref="dialog">
    <div class="dialog-list-wrapper" v-if="!ifNewGroup">
      <div class="dialog-list-item"
        ...
        <div class="dialog-list-icon-wrapper" v-if="isInGroup && item.id === shelfCategory.id">
    ...
    methods: {
      ...
      moveToGroup (group) {
        this.setShelfList(this.shelfList
          .filter(book => {
            if (book.itemList) {
              book.itemList = book.itemList.filter(subItem => this.shelfSelected.indexOf(subItem) < 0)
            }
